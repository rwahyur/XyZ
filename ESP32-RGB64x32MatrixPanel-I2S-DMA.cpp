...
*/
    ...
    i2s_parallel_config_t cfg={
        .gpio_bus={r1_pin, g1_pin, b1_pin, r2_pin, g2_pin, b2_pin, lat_pin, oe_pin, a_pin, b_pin, c_pin, d_pin, e_pin, pinLat2, pinLat3, pinLat4},
        .gpio_clk=clk_pin,
...
/*void RGB64x32MatrixPanel_I2S_DMA::setPanel(uint16_t lebar, uint16_t tinggi){
MATRIX_WIDTH=lebar; MATRIX_HEIGHT=tinggi;
}*/
...
#ifdef tingkatOn
     if((x_coord)==bagiLatch){v|=BIT_LAT2;}
else if((x_coord)==bagiLatch2){v|=BIT_LAT3;}
else if((x_coord)==bagiLatch3){v|=BIT_LAT4;}
else if((x_coord)==PIXELS_PER_ROW-1){v|=BIT_LAT;}
#else 
     if((x_coord)==PIXELS_PER_ROW-1) v|=BIT_LAT;
#endif
        ...
        if((x_coord)==PIXELS_PER_ROW-pir) v|=BIT_OE;    
